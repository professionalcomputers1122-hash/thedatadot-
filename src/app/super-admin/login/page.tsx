    ...
    <main className="min-h-screen bg-[#050b14] text-white flex flex-col justify-center items-center px-6 py-12 relative overflow-hidden">
      ...
      <div className="w-full max-w-md">
        ...
        <div className="rounded-3xl border border-slate-800 bg-slate-900/90 p-8 shadow-2xl backdrop-blur-xl">
          {error && (
            <div className="mb-4 rounded-xl border border-red-500/30 bg-red-950/60 p-3 text-xs text-red-300">
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleLogin} className="space-y-4 text-xs">
            ...
            <button
              type="submit"
              disabled={loading}
              className="mt-2 w-full rounded-xl bg-red-600 py-3.5 text-xs font-bold text-white shadow-lg shadow-red-600/30 transition hover:bg-red-500 active:scale-[0.99] disabled:opacity-50"
            >
        ...
        <p className="mt-6 text-center text-[11px] text-slate-600">
          TLS 1.3 256-Bit Encrypted • All sessions cryptographically audited
        </p>
      </div>
    </main>
  );
