      ...
      <div className="space-y-6 text-xs">
        {notification && (
          <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-xs font-bold text-emerald-800 flex items-center justify-between shadow-2xs">
            ...
            <button
              ...
              className="text-emerald-600 hover:text-emerald-900 ml-4"
            >
              ✕
            </button>
          </div>
        ...
        <div className="rounded-2xl border border-slate-200/80 bg-white p-4 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-2xs">
          ...
          <span className="text-slate-500 font-medium">
            Total B2B Organizations: <strong className="text-slate-900">{companies.length}</strong>
          </span>
        </div>
        ...
        <div className="rounded-3xl border border-slate-200/80 bg-white shadow-2xs overflow-hidden">
          {loading ? (
            <div className="p-16 text-center">
              ...
              <p className="text-slate-500 font-mono text-xs">Loading client organizations...</p>
            </div>
          ) : filtered.length === 0 ? (
            <div className="p-16 text-center">
              <p className="text-sm font-semibold text-slate-700 mb-1">No organizations found</p>
                ...
                <tbody className="divide-y divide-slate-100">
                  {filtered.map((c) => (
                    <tr key={c.id} className="hover:bg-slate-50/70 transition">
                      <td className="px-5 py-4 font-mono font-bold text-blue-600">{c.id}</td>
                      <td className="px-5 py-4 font-bold text-slate-900">{c.name}</td>
                      <td className="px-5 py-4 text-slate-600">{c.industry}</td>
                      <td className="px-5 py-4 text-slate-700 font-medium">{c.plan}</td>
                      <td className="px-5 py-4 font-bold text-slate-900">
                        {c.devicesRecovered} Drives
                      </td>
                      <td className="px-5 py-4 text-slate-500">{c.accountManager}</td>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-xs text-xs">
            <div className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-6 sm:p-8 shadow-2xl text-slate-900">
              <div className="flex justify-between items-center border-b border-slate-100 pb-3 mb-4">
                <h3 className="text-base font-bold text-slate-900">Register Client Organization</h3>
                <button
              ...
              <form onSubmit={handleAddOrg} className="space-y-3.5">
                ...
                <div className="flex justify-end gap-2 pt-3 border-t border-slate-100">
                  ...
                  <button
                    type="submit"
                    className="rounded-xl bg-blue-600 px-5 py-2 font-bold text-white hover:bg-blue-500 transition shadow-md shadow-blue-600/20 cursor-pointer"
                  >
