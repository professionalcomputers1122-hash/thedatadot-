  ...
  return (
      ...
      actions={
        <div className="flex items-center gap-2">
          <Link
            ...
            className="rounded-xl border border-blue-200 bg-blue-50 px-3.5 py-2 text-xs font-bold text-blue-700 hover:bg-blue-100 transition shadow-2xs flex items-center gap-1.5"
          >
    ...
    >
      <div className="space-y-8 animate-in fade-in">
        {/* 1. WELCOME BANNER (CLEAN WHITE ENTERPRISE THEME) */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 p-6 sm:p-8 rounded-3xl border border-slate-200 bg-white shadow-xs">
          <div className="flex items-center gap-4 sm:gap-5">
            {/* AVATAR */}
            <div className="flex h-16 w-16 sm:h-20 sm:w-20 items-center justify-center rounded-2xl bg-gradient-to-br from-red-600 to-rose-700 text-white font-black text-2xl shadow-md shadow-red-600/20 border-2 border-red-500/30 shrink-0">
              EB
            </div>

            <div>
              <div className="flex flex-wrap items-center gap-2 mb-1.5">
                <span className="inline-flex items-center gap-1.5 rounded-full px-3 py-0.5 text-[11px] font-bold border border-red-200 bg-red-50 text-red-700">
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            <Link
              href="/admin/inquiries"
              className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-slate-50 px-4 py-2 text-xs font-bold text-slate-700 shadow-2xs hover:bg-slate-100 hover:text-slate-950 transition"
            >
              <span>🔔</span>
        ...
        {newInquiriesCount > 0 && (
          <div className="rounded-2xl border border-blue-200 bg-blue-50/80 p-4 text-xs text-blue-900 flex flex-wrap items-center justify-between gap-3 shadow-xs">
            <div className="flex items-center gap-2.5">
              <span className="flex h-6 w-6 items-center justify-center rounded-lg bg-blue-600 text-white font-bold text-xs">
                🔔
              </span>
              <span>
                You have <strong className="font-bold text-blue-950">{newInquiriesCount} new client consultation / SLA request(s)</strong> waiting for review and specialist dispatch.
              </span>
            </div>
            <Link
              href="/admin/inquiries"
        ...
        {/* 3. METRICS ROW (CLEAN WHITE CARDS) */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs hover:border-slate-300 transition">
            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500">
              Recovery SLA Success
            </span>
            <div className="mt-2 flex items-baseline justify-between">
              <span className="text-3xl font-black text-emerald-600">99.98%</span>
            ...
            <p className="mt-2 text-[11px] text-slate-500">500+ verified cleanroom jobs</p>
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs hover:border-slate-300 transition">
            ...
            <div className="mt-2 flex items-baseline justify-between">
              <span className="text-3xl font-black text-blue-600">4 / 4</span>
              <span className="text-xs font-mono text-blue-700 bg-blue-50 px-2 py-0.5 rounded border border-blue-200 font-semibold">
                100% Load
              </span>
            </div>
            <p className="mt-2 text-[11px] text-slate-500">PC-3000 Channels 01-04 active</p>
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs hover:border-slate-300 transition">
            ...
            <div className="mt-2 flex items-baseline justify-between">
              <span className="text-3xl font-black text-slate-900">{customerCount || 48}</span>
              <span className="text-xs font-mono text-slate-700 bg-slate-100 px-2 py-0.5 rounded border border-slate-200 font-semibold">
                Enterprise
              </span>
            </div>
            <p className="mt-2 text-[11px] text-slate-500">Active enterprise contracts</p>
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs hover:border-slate-300 transition">
            ...
            <p className="mt-2 text-[11px] text-slate-500">Average triage dispatch &lt; 9 mins</p>
          </div>
        </div>

        {/* 4. MASTER HARDWARE & INCIDENT PIPELINE TABLE (CLEAN WHITE) */}
        <div className="rounded-3xl border border-slate-200 bg-white shadow-xs overflow-hidden">
          {/* TABLE HEADER & FILTER TABS */}
          <div className="p-5 sm:p-6 border-b border-slate-200 flex flex-col gap-4">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <h2 className="text-base font-bold text-slate-950">Master Service Ticket Directory</h2>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2">
              <div className="relative w-full sm:w-80">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2 pl-8 pr-4 text-xs text-slate-900 placeholder-slate-400 outline-none transition focus:border-blue-500 focus:bg-white focus:ring-1 focus:ring-blue-500"
                />
              </div>
                ...
                {filteredTickets.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-5 py-12 text-center text-slate-400">
                      <p className="font-semibold text-slate-600 text-sm">No tickets found</p>
                    ...
                    return (
                      <tr key={t.id} className="hover:bg-slate-50/80 transition">
                        ...
                        <td className="px-5 py-3.5">
                          ...
                          <span className="text-[11px] text-slate-500">{t.customerName}</span>
                        </td>
                        ...
                        <td className="px-5 py-3.5">
                          {isUnassigned ? (
                            <Link
                              href="/admin/tickets"
                              className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-amber-50 text-amber-800 border border-amber-200 text-[10px] font-bold"
                            >
                              <span className="h-1.5 w-1.5 rounded-full bg-amber-500 animate-ping" />
                              Pending Dispatch
                            </Link>
                          ) : (
                            <span className="text-slate-700 font-medium">{t.assignedTech}</span>
                          )}
                        </td>
                        <td className="px-5 py-3.5 text-right">
                          <div className="flex items-center justify-end gap-2">
                            <Link
                              href="/admin/tickets"
                              className="px-2.5 py-1 rounded-lg bg-slate-100 hover:bg-slate-200 text-blue-700 text-xs font-semibold transition"
                            >
                              Dispatch
                            </Link>
                            <Link
                              href={`/technician/tickets/${t.id}`}
                              className="px-2.5 py-1 rounded-lg border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 text-xs font-semibold transition hidden sm:inline-block shadow-2xs"
                            >
                              Workbench →
                            </Link>
                            <button
                              ...
                              className="p-1.5 rounded-lg text-rose-600 hover:text-rose-700 hover:bg-rose-50 transition text-xs cursor-pointer"
        ...
        {/* 5. QUICK SHORTCUTS (CLEAN WHITE TILES) */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
          <Link
            href="/admin/blog"
            className="rounded-3xl border border-slate-200 bg-white p-6 hover:border-slate-300 hover:shadow-md transition group shadow-xs"
          >
            ...
            <h3 className="font-bold text-slate-950 text-sm group-hover:text-blue-600 transition">
          ...
          <Link
            ...
            className="rounded-3xl border border-slate-200 bg-white p-6 hover:border-slate-300 hover:shadow-md transition group shadow-xs"
          >
            <div className="w-10 h-10 rounded-xl bg-red-50 text-red-600 flex items-center justify-center mb-4 border border-red-200">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
